...
async function handleApi(request:Request,env:Env):Promise<Response>{
  ...
  if(path==='/api/focus/sessions'&&request.method==='GET'){
    const r=await env.DB.prepare(`SELECT f.id,f.task_id,t.title task_title,f.started_at,f.ended_at,f.duration_seconds
      FROM focus_sessions f LEFT JOIN tasks t ON t.id=f.task_id
      ORDER BY f.started_at DESC LIMIT 100`).all();
  ...
  if(path==='/api/focus/stop'&&request.method==='POST'){
    const d=await body<{id?:string}>(request); if(!d.id)return json({error:'Falta la sesión.'},400); const row=await env.DB.prepare('SELECT * FROM focus_sessions WHERE id=?').bind(d.id).first<{started_at:string;ended_at:string|null;task_id:string|null}>(); if(!row)return json({error:'Sesión no encontrada.'},404); const ended=isoNow(),seconds=Math.max(1,Math.floor((new Date(ended).getTime()-new Date(row.started_at).getTime())/1000)); await env.DB.prepare('UPDATE focus_sessions SET ended_at=?,duration_seconds=? WHERE id=?').bind(ended,seconds,d.id).run(); if(row.task_id){const total=await env.DB.prepare('SELECT COALESCE(SUM(duration_seconds),0) s FROM focus_sessions WHERE task_id=? AND duration_seconds IS NOT NULL').bind(row.task_id).first<{s:number}>();await env.DB.prepare('UPDATE tasks SET actual_minutes=?,updated_at=? WHERE id=?').bind(Math.round((total?.s||0)/60),ended,row.task_id).run();} await log(env,'focus_stopped',`Terminaste una sesión de enfoque de ${Math.round(seconds/60)} min.`,'focus',d.id,{duration_seconds:seconds}); return json({duration_seconds:seconds});
  }
